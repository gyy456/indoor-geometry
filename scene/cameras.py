...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy,
                 ...
                 image_path, image_name, uid, depth_params=None,
        ...
        self.depth = depth
        # depth_mono_path = image_path.replace('images','depth')
        # depth_mono = Image.open(depth_mono_path)
        # self.depth = depth_mono
        try:
        ...
        _normal_path = self.image_path.replace('images','renders_normal')
        _normal_path = _normal_path.replace('png','jpg')
        if os.path.exists(_normal_path):
            ...
            # normalize normal
            _normal = _normal.permute(1, 2, 0) @ (torch.tensor(np.linalg.inv(self.R)).float())
            _normal = _normal.permute(2, 0, 1)

            self.noraml_gt = _normal.cuda()

            normal_norm = torch.norm(self.noraml_gt, dim=0, keepdim=True)
            self.normal_mask = ~((normal_norm > 1.1) | (normal_norm < 0.9))
            self.noraml_gt= self.noraml_gt/ normal_norm

        depth_path = self.image_path.replace('images','renders_depth')
