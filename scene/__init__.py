...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], partiton=False):
        """b
        ...
        self.test_cameras = {}
        if partiton == True:
            scene_info = sceneLoadTypeCallbacks["ColmapVast"](args.source_path, args.model_path, args.images, args.eval, args.block_json_path)
        ...
        self.train_cameras_list = scene_info.train_cameras
        self.test_cameras_list = scene_info.test_cameras
        ...
        for resolution_scale in resolution_scales:
            ...
            camera_centers = []
            center_rays = []
            for id, cur_cam in enumerate(self.train_cameras_list ):
                cur_cam = cameraList_from_camInfos(self.train_cameras_list [id:id+1], resolution_scale, args)[0]
            ...
            with open(os.path.join(self.model_path, "multi_view.json"), 'w') as file:
                for id, cur_cam in enumerate(self.train_cameras_list ):
                    cur_cam_1 = cameraList_from_camInfos(self.train_cameras_list[id:id+1], resolution_scale, args)[0]
                    ...
                    json_d = {'ref_name' : cur_cam_1.image_name, 'nearest_name': []}
                    for index in sorted_indices[:multi_view_num]:
                        self.train_cameras_list[id].nearest_id.append(index)
                        self.train_cameras_list[id].nearest_names.append(scene_info.train_cameras[index].image_name)
                        json_d["nearest_name"].append(scene_info.train_cameras[index].image_name)
            ...
            with open(os.path.join(self.model_path, "multi_view.json"), 'w') as file:
                for id, cur_cam in enumerate(self.train_cameras[resolution_scale] ):
                    ...
                    for index in sorted_indices[:multi_view_num]:
                        ...
                        cur_cam.nearest_names.append(scene_info.train_cameras[index].image_name)
                        json_d["nearest_name"].append(scene_info.train_cameras[index].image_name)
                    json_str = json.dumps(json_d, separators=(',', ':'))
                    file.write(json_str)
                    file.write('\n')
