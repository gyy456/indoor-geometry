...
from utils.graphics_utils import patch_offsets, patch_warp, depth_propagation, check_geometric_consistency
import sys
...
def training(dataset, opt, pipe, model_dpt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    scene = Scene(dataset, gaussians,partiton=False)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration > opt.multi_view_weight_from_iter:
            ...
            nearest_cam = None if len(viewpoint_cam.nearest_id) == 0 else cameraList_from_camInfos(scene.train_cameras_list[index_near:index_near+1], 1.0, dataset)[0]
        ...
        with torch.no_grad():
            ...
            if opt.depth_loss and iteration > propagated_iteration_begin and iteration < propagated_iteration_after and (iteration % opt.propagation_interval == 0 and not propagation_dict[viewpoint_cam.image_name]):
                # if opt.depth_loss and iteration > propagated_iteration_begin and iteration < propagated_iteration_after and (iteration % opt.propagation_interval == 0):
                    ...
                    propagated_depth, normal = depth_propagation(viewpoint_cam, projected_depth, scene.train_cameras_list, src_idxs, opt.patch_size, dataset)
                    ...
                    for idx, src_idx in enumerate(src_idxs):
                        src_viewpoint = cameraList_from_camInfos(scene.train_cameras_list[src_idx:src_idx+1], 1.0, dataset)[0]
                        #c2w
                        ...
                        #get the src_depth first
                            src_depth, src_normal = depth_propagation(src_viewpoint, src_projected_depth, scene.train_cameras_list, src_idxs, opt.patch_size,dataset)
