...
def render_set(model_path, name, dataset,iteration, train_cameras_list, scene, gaussians, pipeline,
    ...
    model.load_state_dict(torch.load('checkpoints/depth_anything_v2_vitl.pth', map_location='cuda'))
    model.to('cuda')
    model.eval()
    ...
    for idx, view in enumerate(tqdm(train_cameras_list, desc="Rendering progress")):
        ...
        gt = cv2.imread(view.image_path)

        _, H, W = gt.shape
        # gt = torch.tensor(gt, device="cuda")  # h w 3
        depth = model.infer_image(gt) # HxW raw depth map


        cv2.imwrite(os.path.join(render_depth_path, view.image_name + ".jpg"), depth)
